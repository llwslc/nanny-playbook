...
<script>
  // 只有带鼠标的设备（桌面/笔记本——会拿去打印的设备）才排成 A4 预览（.paged）；
  // 手机、平板没人打印，按普通网页渲染（.plain），正常字号、自动换行。
  // 类要在首帧前挂上：.paged 的「藏住未分页正文」规则靠它，不闪裸文档。
  var paged = matchMedia('(hover: hover) and (pointer: fine)').matches;
  document.documentElement.className = paged ? 'paged' : 'plain';
  // auto:false 时排版引擎装载后不动手。after 是排版完成的信号：
  // 无头浏览器出 PDF 时等它，别在排到一半时出片。
  window.PagedConfig = { auto: paged, after: function () { window.__pagedjsDone = true; } };
</script>
...
<style media="screen">
  /* 屏幕上的样式。media="screen" 一石二鸟：打印时浏览器天然忽略；
     排版引擎收集样式表时也明确跳过 media~='screen' 的表——
     所以这一套从加载那一刻就生效，且不会被引擎重写。 */

  /* 桌面（.paged）：灰底上一叠居中的 A4 纸，预览即打印效果 */
  .paged body { background: #d9d9d9; }
  .pagedjs_page { background: #fff; box-shadow: 0 1px 6px rgba(0,0,0,.28); margin: 16px auto; }
  /* 排版引擎启动后才把正文收进页面容器；在那之前把原始正文藏住，
     避免首帧闪一下未分页的裸文档 */
  .paged body > :not(.pagedjs_pages) { display: none; }

  /* 手机 / 平板（.plain）：普通网页，正常字号、自动换行，宽表格横向滚动。
     overflow-wrap: anywhere——超长的填空线（____…）是不可断串，不许它断
     就会从段落里漏出去，把整页撑宽，右边多出一条空白。 */
  .plain body { max-width: 820px; margin: 0 auto; padding: 24px 16px; font-size: 15px; line-height: 1.75; overflow-wrap: anywhere; }
