...
<style>
  ...
  * { box-sizing: border-box; }
  ...
  /* 屏幕预览的样式（灰底、白纸、居中）不在这里——排版引擎会重写这个样式表，
     @media screen 的块会被它吃掉。见文末排版完成回调：那里注入的才归浏览器管。 */
</style>
</head>
<body>
${bodyHtml}
<script>
  window.PagedConfig = {
    after: () => {
      // 屏幕预览样式（灰底、白纸居中、页间距）。必须等排版结束再注入：
      // head 里的样式表会被排版引擎重写，@media screen 的块活不下来。
      // margin auto 在窄屏上自动退化成 0——页面比屏宽时靠左、可滚动，不会两边裁切。
      const s = document.createElement('style');
      s.textContent = '@media screen { body { background: #d9d9d9; } ' +
